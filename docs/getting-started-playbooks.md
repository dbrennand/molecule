...
Use this page when the primary deliverable is an Ansible playbook project and the main question is whether its root playbooks work with its inventory and project configuration.

Choose [Test a role](getting-started-roles.md) when the main question is whether one role produces the expected result. Choose [Test a collection](getting-started-collections.md) when collection packaging or multiple collection component types are the focus.
